...
//Global variables for Adjective to uncause the invocation of new object each time
...
//Global variable for nouns to uncause the invocation of object each time
...
switch (size) {
    ...
    default:
      ...
      //Use of JS api for getting element to fasten the process
var ranpizzacon = document.getElementsByClassName("randomPizzaContainer");
var len = ranpizzacon.length; //To save the length of array
    for (var i = 0; i < len ;i++) {
      //Improved way of setting width
      ranpizzacon[i].style.width = newwidth + "%";
    }
}
...
// The following JS code executed one which better to include inside loop
...
//Better to call class by Name rather than queryselector
  var items = document.getElementsByClassName('mover');
  len1 = items.length; //calculate length of item array
...
document.addEventListener('DOMContentLoaded', function() {
  iHeight = window.screen.height; //calculate height
  var cols = 8;
  var s = 256;  //smallest expected height
  var rows = Math.floor(iHeight / s); //calculate the no. of rows for pizza
  var no_pizza = cols*rows; //calculate the no. of pizza


  for (var i = 0; i < no_pizza; i++) {
    var elem = document.createElement('img'); //Not moved out these two because doing so
    var movingPizzas = document.getElementById('movingPizzas1'); //it is not working
